...
The end-to-end pipeline runs on synthetic data (encoder, connector, anchor,
contrastive loss, retrieval metric). Real data adapters, a feature cache, and
PEFT adaptation follow (see roadmap).
...
ablation-heavy methodology affordable.

## Planned layout

```
configs/      Hydra configs — the single source for experiments (config sweeps)
src/cogfm/
  ...
  binding/    binding training loop (reads cached features)
  adaptation/ Phase-2: soft-prompts + PEFT
  eval/       subject-CV protocol, retrieval (floor/ceiling), downstream, stats
  registry.py name -> class, for swapping components in sweeps
...
- **M5** Configuration sweeps + mandatory ablations
